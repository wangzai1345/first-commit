...
function Captain() {
    ...
    // 新增队员信息函数
    const addTeamMember = () => {
        setTeamMembers([...teamMembers, { id: '', name: '', college: '' }]);
    };
    ...
    return (
        <div>
            ...
            <br />
            <h3>队员信息</h3>
            <button onClick={addTeamMember}>添加队员</button>
            {teamMembers.map((member, index) => (
                <div key={index}>
                    ...
                    <input
                        type="text"
                        value={member.id}
                        onChange={(e) => handleMemberInputChange(index, 'id', e.target.value)}
                        placeholder="请输入队员学号"
                    />
                    <br />
                    <label>队员姓名：</label>
                    ...
                    <input
                        ...
                        onChange={(e) => handleMemberInputChange(index, 'college', e.target.value)}
                        placeholder="请输入队员学院"
                    />
                    <br />
                </div>
            ))}
            <button onClick={submitTeamInfo}>提交队伍信息</button>
            <br />
...
/*              const [createTime, setCreateTime] = useState(''); */
             ...
             // 处理文件选择
             const handleFileSelect = (e) => {                                     //e是一个事件对象
                 const file = e.target.files[0];                                   //取e事件对象的第一个文件，赋值给file
         
                 if (file && allowedTypes.includes(file.type)) {                   //file存在且file的类型在allowedTypes数组中，数组allowedTypes是允许的文件类型
                     setSelectedFile(file);                      //设置selectedFile为file,在line 78的useState
                     setUploadStatus('');//设置上传状态为空
                     const reader = new FileReader();                               //FileReader对象用于将文件读取为内存中的数据
